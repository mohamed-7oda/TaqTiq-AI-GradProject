...
try:
    ...
    raise ImportError(f"TensorFlow/Keras not found: {e}\nRun: pip install tensorflow==2.20.0")
...
class VideoFeatureExtractor():
    def __init__(self,
        ...
        if "TF2" in self.back_end:
            ...
            # NEW
            base_model = tf.keras.applications.ResNet152(include_top=True,
                                                        weights='imagenet',
                                                        input_tensor=None,
                                                        input_shape=None,
                                                        pooling=None,
                                                        classes=1000)
            self.model = tf.keras.Model(base_model.input,
    ...
    def extractFeatures(self, path_video_input, path_features_output, start=None, duration=None, overwrite=False):
        ...
        if "TF2" in self.back_end:
            ...
            logging.info(f"frames {raw_frames.shape}, fps={raw_frames.shape[0]/duration}")

            # ---- Batched preprocessing + prediction (low memory) ----
            batch_size = 32          # lower this if you still get OOM (try 16 or 8)
            n_frames = raw_frames.shape[0]
            features_list = []

            logging.info(f"Processing {n_frames} frames in batches of {batch_size}...")
            for start_idx in tqdm(range(0, n_frames, batch_size), desc="Extracting features"):
                end_idx = min(start_idx + batch_size, n_frames)
                # Cast + preprocess only this batch (avoid 3 GB allocation)
                batch = raw_frames[start_idx:end_idx].astype(np.float32, copy=False)
                batch = preprocess_input(batch)
                feats_batch = self.model.predict(batch, batch_size=batch_size, verbose=0)
                features_list.append(feats_batch)
                # Free memory
                del batch, feats_batch

            features = np.concatenate(features_list, axis=0)
            del features_list, raw_frames
