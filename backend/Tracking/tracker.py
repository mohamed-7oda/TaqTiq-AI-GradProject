...
class Tracker:
    ...
    def get_object_tracks_streaming(self, video_path, read_from_stub=False,
        ...
        if read_from_stub and stub_path is not None and os.path.exists(stub_path):
            with open(stub_path, 'rb') as f:
                data = pickle.load(f)
            # Support both old (tracks-only) and new (tracks, first_frame) stubs
    ...
    def draw_pass_counter(self, frame, team_passes, team_colors=None):
        ...
        t1 = team_passes.get(1, 0)
        t2 = team_passes.get(2, 0)
        ...
        for row, (tid, passes) in enumerate([(1, t1), (2, t2)]):
            ...
            cv2.putText(frame, f"Team {tid} Passes: {passes}",
                        (tx + sq + int(6*s), ty),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.9*s, (0, 0, 0), max(1, int(2*s)))
        return frame
